...
from dreamboard.actions.menu_structure import MENU_SEPARATOR
...
@patch('dreamboard.actions.mixin.menu_structure')
@patch('dreamboard.actions.mixin.actions')
@patch('dreamboard.actions.mixin.KeyboardSettings.get_shortcuts')
def test_create_actions(
    ...
    assert widget.dreamb_actions['foo'] == qaction
...
def test_create_actions_with_group(actions_mock, menu_mock, qapp):
    ...
    assert len(widget.actions()) == 1
    qaction = widget.actions()[0]
    assert widget.dreamb_actiongroups['bar'] == [qaction]
...
def test_build_menu_and_actions_with_actions(actions_mock, menu_mock, qapp):
    ...
    with patch('PyQt6.QtWidgets.QMenu.addAction') as add_mock:
        ...
        add_mock.assert_called_once_with(widget.dreamb_actions['foo'])
...
def test_actiongroup_set_enabled(actions_mock, menu_mock, qapp):
    ...
    assert widget.dreamb_actions['foo'].isEnabled() is True
    assert widget.dreamb_actions['bar'].isEnabled() is False
...
def test_create_recent_files_more_files_than_shortcuts(
    ...
    widget.settings.get_recent_files.return_value = [
        os.path.abspath(f'{i}.dreamb') for i in range(15)]
    menu_mock.__iter__.return_value = [{
        'menu': 'Open &Recent',
    ...
    assert qaction1.text() == '0.dreamb'
    assert qaction1.shortcut() == 'Ctrl+1'
    assert qaction1.isEnabled() is True
    assert widget.dreamb_actions['recent_files_0'] == qaction1
    qaction10 = widget.actions()[9]
    assert qaction10.text() == '9.dreamb'
    assert qaction10.shortcut() == 'Ctrl+0'
    assert qaction10.isEnabled() is True
    assert widget.dreamb_actions['recent_files_9'] == qaction10
    qaction15 = widget.actions()[-1]
    assert qaction15.text() == '14.dreamb'
    assert qaction15.shortcut() == ''
    assert qaction15.isEnabled() is True
    assert widget.dreamb_actions['recent_files_14'] == qaction15
...
def test_create_recent_files_fewer_files_than_shortcuts(
    ...
    widget = FooWidget()
    widget.settings.get_recent_files.return_value = [
        os.path.abspath(f'{i}.dreamb') for i in range(5)]
    menu_mock.__iter__.return_value = [{
        'menu': 'Open &Recent',
        'items': '_build_recent_files',
    }]
    ...
    assert qaction5.text() == '4.dreamb'
    assert qaction5.shortcut() == 'Ctrl+5'
    assert qaction5.isEnabled() is True
    assert widget.dreamb_actions['recent_files_4'] == qaction5
...
def test_update_recent_files(actions_mock, menu_mock, triggered_mock, qapp):
    ...
    assert len(widget.actions()) == 1
    qaction1 = widget.actions()[0]
    assert qaction1.text() == 'foo.dreamb'
    ...
    assert len(widget.actions()) == 1
    qaction1 = widget.actions()[0]
    assert qaction1.text() == 'bar.dreamb'
